...
from ..util import check_zigbee_exist, check_device_state, check_device_exist, send_command
from ...exceptions import exception, DeviceNotFound, DeviceOffline, InvalidPayload, Unsupported
from ...zigbee import type_exist, format_validation, value_validation
from ...zigbee.data_type import get_bytes, data_type_value_table, data_type_table
...
class ZigbeesResource(Resource):
    ...
    def get(self):
        ...
        def handle():
            paras = request.args
            devices = DBZigbee(**paras).retrieve()
            for device in devices:
                device['extended_pan_id'] = int(device['extended_pan_id'], 16)
            return devices
...
class ZigbeeResource(Resource):
    ...
    def get(self, mac):
        @exception
        def handle():
            device = DBZigbee(mac=mac).retrieve()
            if not device:
...
class ZigbeeAttributesResource(Resource):
    ...
    def get(self, mac):
        ...
        def handle():
            ...
            params = {}
            for key in request.args:
                if key not in ['endpoint', 'cluster', 'server', 'attribute', 'manufacturer', 'manufacturer_code']:
                    raise InvalidPayload(key)
                if key == 'server':
                    params[key] = bool(int(request.args[key]))
            ...
            return send_command(ip=device.get('ip'), mac=mac, command={'attribute':params})
