...
class XMODEM(object):
    ...
    async def send(self, stream, retry=16, timeout=60, quiet=False, callback=None):
        '''
        ...
        while True:
            ...
            while True:
                ...
                char = await self.getc(1, timeout)
                if char == ACK:
                    success_count += 1
    ...
    async def recv(self, stream, crc_mode=1, retry=16, timeout=60, delay=1, quiet=0):
        '''
        ...
        while True:
            ...
            seq1 = await self.getc(1, timeout)
            if seq1 is None:
                self.log.warn('getc failed to get first sequence byte')
                seq2 = None
            else:
                seq1 = ord(seq1)
                seq2 = await self.getc(1, timeout)
                if seq2 is None:
            ...
            if not (seq1 == seq2 == sequence):
                ...
                self.log.error('expected sequence %d, '
                               ...
                               sequence, seq1, seq2)
                await self.getc(packet_size + 1 + crc_mode)
            else:
                # sequence is ok, read packet
                # packet_size + checksum
                data = await self.getc(packet_size + 1 + crc_mode, timeout)
            ...
            while True:
                ...
                data = await self.getc(1, timeout=1)
                if data is None:
                    break
            self.putc(NAK)
            # get next start-of-header byte
